...
{
    public class DataSet2DAOimpl : IDataSet2DAO
    {
        public IEnumerable<DataSet2> FindAll(int wid, string code, DateTime timefrom, DateTime timeto)
        {
            string query = "select wid, code, value, time from dataset2 " +
                "where time between :timefrom and :timeto " +
                "and code = :code " +
                "and wid = :wid";

            List<DataSet2> dataset2List = new List<DataSet2>();
            ...
            using (IDbConnection connection = ConnectionUtil_Pooling.GetConnection())
            {
                ...
                using (IDbCommand command = connection.CreateCommand())
                {
                    ...
                    using (IDataReader reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            DataSet2 ds = new DataSet2(reader.GetInt32(0), reader.GetString(1), reader.GetInt32(2), reader.GetDateTime(3));
                            dataset2List.Add(ds);
                        }
                    }
                }
        ...
        public IEnumerable<DataSet2> FindCodes(string code)
        {
            string query = "select wid, code, value, time from dataset2 " +
                "where code = :code";
        ...
        public int Save(DataSet2 entity)
        {
            using (IDbConnection connection = ConnectionUtil_Pooling.GetConnection())
            {
                connection.Open();

                string insertSql = "insert into dataset2 (wid, code, value, time) " +
        ...
        {
            string query = "delete from dataset2";
